...
class UserManage extends Component{
	...
	async getDataSource(data){
		...
		const res=await axios.post('/vo/users',data)
	...
	async handleModalOK(data){
		const { orderId,queryForm,modalData }=this.state
		let res={}

		if(orderId===0){
			res=await axios.post('/vo/user',{...data,moduleID:1})
		}else{
			res=await axios.put(`/vo/user/${orderId}`,{...data,id:orderId,salt:modalData.salt})
		}
		...
		if(selectedRow.id){
			...
			if(obj.btnIndex===3){
				Modal.confirm({
				    ...
				    cancelText: '取消',
				    async onOk(){
				    	const res=await axios.delete(`/vo/user/${modalData.id}`)
				    	if(res.code===200){
				    		message.success('操作成功')
	...
	render(){
		...
		const footerBtns=[
				...
				{text:'重置密码',onClick:this.handleFooterBtnClick.bind(this,{btnIndex:2,text:'重置密码'})},
				{text:'删除',onClick:this.handleFooterBtnClick.bind(this,{btnIndex:3,text:'您确定要删除该条数据吗？'})},
			]
		...
		return (
			<div className='container'>
				<QueryForm 
					...
					initData={initData}
					formItems={TableColums.UserManageSearch}
				/>
				<TableBar 
					ref={(table)=>{this.table=table}}
					columns={this.renderColumns(TableColums.UserManage)}
				...
				{modalIndex===1 && 
					<ModalBar.Form 
			      		...
			      		formItems={TableColums.UserManage}
			      	/>
				}
				{modalIndex===2 && 
					<ModalBar.Detail 
			      		...
			      		onCancel={this.handleCancel.bind(this)}
			      		modalData={modalData}
			      		modalItems={TableColums.UserManage}
			      	/>
				}
			</div>
		)
