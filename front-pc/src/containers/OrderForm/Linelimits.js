...
@observer
class LineLimits extends Component{
	constructor(props){
		 super(props)
	...
	async getDataSource(data){
		...
		const res=await axios.post('/vo/search/line_limit',data)
	...
	async handleModalOK(data){
		...
		if(orderId===0){
			res=await axios.post('/vo/line_limit',data)
		}else{
			res=await axios.put(`/vo/line_limit/${orderId}`,data)
		}
		...
		if(selectedRow.id){
			...
			if(obj.btnIndex===2){
				Modal.confirm({
				    ...
				    cancelText: '取消',
				    async onOk(){
				    	const res=await axios.delete(`/vo/line_limit/${modalData.id}`)
				    	if(res.code===200){
				    		message.success('操作成功')
		...
		return (
			<div className='container'>
				<QueryForm 
					...
					initData={initData}
					formItems={TableColums.LineLimitsSearch}
				/>
				<TableBar 
					ref={(table)=>{this.table=table}}
					columns={this.renderColumns(TableColums.LineLimits)}
				...
				{modalIndex===1 && 
					<ModalBar.Form 
			      		...
			      		formItems={TableColums.LineLimits}
			      	/>
				}
				{modalIndex===2 && 
					<ModalBar.Detail 
			      		...
			      		onCancel={this.handleCancel.bind(this)}
			      		modalData={modalData}
			      		modalItems={TableColums.LineLimits}
			      	/>
				}
			</div>
		)
