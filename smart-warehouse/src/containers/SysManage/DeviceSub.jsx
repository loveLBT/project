...
class DeviceSub extends Component {
	state = {
		dataList: [],
		formItems: [],
		tableLoading: true,
	...
	getHeader = async () => {
		let res = await api.getHeader('tianjiazishebei');
	...
	getList = async (pageNum) => {
		...
		let res = await api.getListSubEquipmentDetail(pageNum);
		let dataList = [];
	...
	getFormHeader = async () => {
		let res = await api.getFormHeader('zishebeiliebiao');
	...
	handleFormSubmit(e) {
		...
		this.form.validateFields(async (err, values) => {
			if (!err) {
				console.log(values)
				let res = await api.insertSubEquipment(values);
				if (res.code === 200) {
	...
	handleModalEdit = async (e, j) => {
		this.table.form.validateFields(async (err, values) => {
			if (!err) {
				...
				if (flag) {
					let res = await api.updateSubEquipment(e.sub_equipment_id, values);
					if (res.code === 200) {
	...
	del = async (recode) => {
		let res = await api.deleteSubEquipment(recode.sub_equipment_id);
		if (res.code === 200) {
