...
class DeviceTypeManage extends Component {
	...
	getHeader = async () => {
		let res = await api.getHeader('xinzengshebeileixing');
	...
	getList = async (pageNum) => {
		...
		let res = await api.getListEquipmentType(pageNum);
		let dataList = [];
	...
	render() {
		return (
			<div>
				<div className="subTit">新增设备类型</div>
